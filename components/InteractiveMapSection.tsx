...
const InteractiveMapSection: React.FC<InteractiveMapSectionProps> = ({ copy, cta }) => {
  return (
    <section id="service-area-map" className="py-20 px-4 sm:px-6 lg:px-8 bg-black">
      <div className="max-w-7xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mt-16 items-center">
          ...
          <div className="relative w-full aspect-video rounded-lg shadow-2xl border-2 border-gold-accent overflow-hidden">
            <img
          ...
          <div className="bg-neutral-900 p-8 rounded-lg shadow-xl text-left">
            <h3 className="text-3xl font-bold text-gold-accent mb-6">Explore Our Key Service Areas</h3>
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-xl">
              {SERVICE_AREAS_DATA.map((area) => (
                <li key={area.id}>
                  ...
                  >
                    <span className="inline-block w-2.5 h-2.5 bg-gold-accent rounded-full mr-3 group-hover:bg-light-gold-accent transition-colors duration-300"></span>
                    {area.name}
                  </Link>
                </li>
              ))}
