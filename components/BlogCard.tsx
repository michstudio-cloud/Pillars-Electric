...
const BlogCard: React.FC<BlogCardProps> = ({ post }) => {
  return (
    <Link to={`/blog/${post.id}`} className="block bg-neutral-900 rounded-lg shadow-xl hover:shadow-2xl group transform hover:-translate-y-1 transition-all duration-300 ease-in-out overflow-hidden">
      <div className="relative h-56 w-full"> {/* Increased image height */}
        ...
        />
        <div className="absolute inset-0 bg-black opacity-70 group-hover:opacity-85 transition-opacity duration-300"></div> {/* Slightly darker overlay on hover */}
      </div>
      <div className="p-8"> {/* Increased padding */}
        ...
        <p className="text-white text-lg leading-relaxed mb-4"> {/* Larger description text with relaxed line-height */}
