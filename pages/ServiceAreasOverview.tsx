  ...
  return (
    <div className="min-h-screen pt-20"> {/* pt-20 to offset fixed header */}
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-black">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 mt-16"> {/* Adjusted gap and margin-top */}
          {localAreas.map((area) => (
            <Link
              to={`/service-areas/${area.id}`}
              key={area.id}
              className="block bg-neutral-900 p-8 rounded-lg shadow-xl hover:shadow-2xl group transform hover:-translate-y-1 transition-all duration-300 ease-in-out text-center"
            >
              <h3 className="text-2xl font-bold text-gold-accent mb-3 group-hover:text-light-gold-accent transition-colors duration-300 leading-tight">
                {area.name}
              </h3>
        ...
        <div className="mt-20"> {/* Increased margin-top */}
          ...
          <div className="max-w-7xl mx-auto mt-12 flex flex-wrap justify-center gap-8"> {/* Adjusted gap */}
            {statewideAreas.map((area, index) => (
              <div key={index} className="bg-neutral-900 px-10 py-5 rounded-lg shadow-md text-2xl font-bold text-white"> {/* Increased padding and font size */}
                {area}
              </div>
            ))}
          </div>
